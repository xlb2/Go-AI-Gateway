...
CLI现接入只读项目工具list_files/read_file/search_files，启动可用 `bash scripts/run-agent.sh -user 1 -workspace .`。限定目录、敏感路径排除、输出预算及验收边界见 [CLI文档](docs/cli.md)；不开放写文件或通用命令执行。

完整操作、调用链和验证边界见 [CLI 使用与实现交接](docs/cli.md)。下方早期验收数字为历史记录，最新状态以该文档及 test/README.md 顶部为准。

提交前最新验收（2026-09-22）：用户WSL带knowledgeintegration全量189项通过、0跳过；本地静态检查和CLI构建通过。真实模型已使用源码工具；引用完整性与回答长度仍有不足。自动多行粘贴及输入编辑后置，现有显式/paste模式和运行中取消仍需真实终端验收。
